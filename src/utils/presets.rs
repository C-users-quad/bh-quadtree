...
use crate::{
    physics::constants::{CENTRAL_RADIUS, G, NUM_PARTICLES, ORBITAL_RADIUS},
...
impl Presets {
    ...
    pub fn get_particles(&self) -> Vec<Particle> {
        match self {
            ...
            Self::Collapse => gen_collapse(Vec2::ZERO, 2000.0, 100000.0),
            ...
            Self::BlackHole => gen_black_hole(),
            Self::Square => gen_square(),
        }
    }
...
// particles arranged in a grid contained by a square
pub fn gen_square() -> Vec<Particle> {
    let mut result: Vec<Particle> = Vec::new();
    let num_p_side = NUM_PARTICLES.isqrt() as f32;
    let side_len = 5000.0;
    let ds = side_len / num_p_side;
    let topleft = Vec2::ONE * side_len * -0.5;
    for i in 0..num_p_side as u32 {
        for j in 0..num_p_side as u32 {
